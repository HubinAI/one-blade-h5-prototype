...
describe('爆发节奏', () => {
  ...
  it('10 目标 → 3 组 + combo 大', () => {
    const g = mk();
    (g as any).clusterSlashFloats(Array.from({length:10},(_,i)=>({damage:125,x:50+i*25,y:400,isKill:false})),100,'s');
    expect((g as any)._burstTimers.length).toBeGreaterThanOrEqual(4);
  });
});
