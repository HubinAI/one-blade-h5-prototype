...
import { buildReactiveSlashGeometry, drawReactiveSlashDebug, capsuleHitsEllipse, type ReactiveSlashGeometry } from "./systems/reactiveSlashGeometry";
...
export class Game {
  ...
  constructor(level: LevelConfig, onFinish: FinishCallback, onReviveOffer?: ReviveOfferCallback, runMode?: "normal" | "challenge", bossFlow?: "legacy" | "reactive" | "strategySlice") {
    ...
    if (__E2E_BRIDGE__) {
      ...
      this._e2eInstanceId = instanceId;  // V0723016复审: 存为字段供 GameCanvas cleanup 守卫
      ...
      // V0723016复审: 程序化完成当前 threat objective（不等 timeout，避免玩家被弹幕打死）
      ...
      // V0723016最终收口P0-1: 正常操作斩弹/反射（推进目标但不跳过，替代 forceCompleteObjective 作弊）
      const forceCutProjectileForTest = (kind?: string): boolean => {
        const rc = self.reactiveController;
        if (!rc || rc.phase !== "armor_threat") return false;
        // V0723016最终收口P0-1: 自动根据护甲选择弹幕 kind（右肩用 reflective 触发反射）
        const armorIdx = rc.getActiveArmorIndex();
        const autoKind = kind ?? (armorIdx === 1 ? "reflective" : "normal");
        // 推进 spawn 间隔（递增 waveCount + spawn 弹幕）
        const si = REACTIVE_BOSS_CONFIG.threatSpawn.intervalBase + REACTIVE_BOSS_CONFIG.threatSpawn.intervalJitter;
        rc.update(si + 0.01);
        // spawn 测试弹幕（静止，位于玩家可斩区域）
        const p = rc.spawnProjectileForTest(autoKind as any, DESIGN_WIDTH / 2, 320, 0, 0);
        // 斩弹（用 reactiveBladeMax 确保 burst，reflective 会触发反射）
        const slashId = `e2e_cut_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
        const segA = { x: p.x - 30, y: p.y - 30 };
        const segB = { x: p.x + 30, y: p.y + 30 };
        const angle = Math.atan2(segB.y - segA.y, segB.x - segA.x);
        const e2eMomentum = createBladeMomentumState(self.reactiveBladeMax, self.reactiveBladeMax);
        const geometry = buildReactiveSlashGeometry(segA, segB, angle, 30, 3, slashId, e2eMomentum);
        rc.setProgrammaticSlashEnergy(self.reactiveBladeMax);
        rc.resolveGeometry(geometry);
        rc.finishSlash(slashId, e2eMomentum, 100, self.reactiveBladeRunModifiers);
        return true;
      };
      ...
      (window as any).__ONE_BLADE_E2E__ = {
        instanceId,
        levelId: level.id,
        forceCompleteObjective,  // V0723016复审: 仅供独立测试桥能力单测
        forceCutProjectileForTest,  // V0723016最终收口P0-1: 正常操作斩弹/反射
        ...
        getState: () => {
          // V0723016-S1: 策略切片模式
          if (self.gameMode === "strategySlice" && self.strategySliceController) {
            const snap = self.strategySliceController.getSnapshot();
            return {
              phase: snap.phase,
              cycleIndex: snap.cycleIndex,
              coreState: snap.coreState,
              windowType: snap.windowType,
              sliceElapsed: snap.sliceElapsed,
              armorDurability: snap.armorDurability,
              inputLocked: snap.inputLocked,
              energy: self.energy,
              gameMode: self.gameMode,
              gameInstanceId: instanceId,
              cleanClears: snap.cleanClears,
              chargedReflects: snap.chargedReflects,
              overloads: snap.overloads,
              windowAttacks: snap.windowAttacks,
              windowSkips: snap.windowSkips,
              dangerWrongCuts: snap.dangerWrongCuts,
            };
          }
          ...
          return {
            ...
            lastReactiveExitSnapshot: self._lastReactiveExitSnapshot ?? undefined,
            // V0723016复审P0-3: gameInstanceId + 时间轴遥测（boss 模式）
            gameInstanceId: instanceId,
            breakthroughSuccessTs: self._breakthroughSuccessTs,
          };
        },
        ...
        // V0723016-S1: E2E 设置能量（供策略切片测试使用）
        setEnergyForTest: (value: number) => {
          self.energy = Math.max(0, Math.min(self.reactiveBladeMax, value));
        },
      };
    }
    ...
    // 首局教学检测（Boss模式不触发）
    this.isFirstRun = (this.gameMode === "normal") && (level.id === 1 || level.id === 10001) && !window.localStorage.getItem("one_blade_first_run_done");
    if (this.isFirstRun) {
  ...
  /** V0723016-S1.2: 策略切片退出（不调用 finish，不触发突破流程） */
  exitStrategySlice(): void {
    if (this.finished) return;
    this.finished = true;
    const snap = this.strategySliceController?.getSnapshot();
    const result = {
      win: true,
      ...
      strategySliceSummary: snap ? {
        ...
        windowSkips: snap.windowSkips,
        dangerWrongCuts: snap.dangerWrongCuts,
        totalTime: snap.sliceElapsed,
        cycleOutcomes: snap.cycleOutcomes,
        totalArmorDamage: snap.totalArmorDamage,
  ...
  /** V0723016复审P0-3: 真正突破成功时间（由 Game 在 bossController 进入 execution_success 时记录） */
  private _breakthroughSuccessTs: number | null = null;
  ...
  /** V0723016-S1.1: 策略切片模式渲染（复用 reactive 外观管线） */
  private renderStrategySliceMode(ctx: CanvasRenderingContext2D): void {
    const ssc = this.strategySliceController;
    if (!ssc) return;
    const snap = ssc.getSnapshot();
    ...
    for (const f of ssc.getFeeders()) drawFeederProjectile(ctx, f, this.elapsed);

    // 4. 核心弹
    drawCoreProjectile(ctx, ssc.getCoreProjectile(), snap.coreState, this.elapsed);

    // 5. 危险弹
    for (const p of ssc.getProjectiles()) {
      if (p.kind === "dangerous") drawDangerProjectile(ctx, p, this.elapsed);
    }
    ...
    // 11. HUD：刀势条
    const bm = this.getReactiveBladeMomentum();
    drawEnergyBar(ctx, this.energy, this.reactiveBladeMax, DESIGN_WIDTH / 2 - 90, DESIGN_HEIGHT - 56, 180, 18, bm.band);
  ...
  private drawStrategySliceDebug(ctx: CanvasRenderingContext2D, snap: import("./systems/BossStrategySliceController").SliceSnapshot): void {
    ctx.save();
    ctx.fillStyle = "rgba(0, 0, 0, 0.65)";
    ctx.fillRect(DESIGN_WIDTH - 128, 8, 120, 72);
    ctx.fillStyle = "#5bc0ff";
    ctx.font = '9px "Consolas", monospace';
    ctx.textAlign = "right";
    const lines = [`${snap.phase} c${snap.cycleIndex}`, `core:${snap.coreState ?? "-"}`, `win:${snap.windowType}`, `${snap.sliceElapsed.toFixed(1)}s`];
    for (let i = 0; i < lines.length; i++) ctx.fillText(lines[i], DESIGN_WIDTH - 16, 22 + i * 14);
    ctx.restore();
  }
  ...
  private updateBossMode(scaledDt: number, frameDt: number): void {
    ...
    this.updateBossSpawn();
    this.updateBossController(scaledDt);
    // V0723016复审P0-3: 检测 bossController 进入 execution_success（真正突破成功时刻）
    if (this.bossController?.phase === "execution_success" && this._breakthroughSuccessTs === null) {
      this._breakthroughSuccessTs = this.elapsed;
    }
  ...
  /** V0723016-S1: 策略切片模式主更新 */
  private updateStrategySliceMode(scaledDt: number): void {
    const ssc = this.strategySliceController;
    if (!ssc) return;
    // S1.7: slice_complete 直接路由结果页，不显示 Canvas 总结
    if (ssc.phase === "slice_complete") {
      if (!this.finished) this.exitStrategySlice();
      return;
    }
    this.elapsed += scaledDt;
    this.updateActiveSlash(scaledDt);
  ...
  private isCurrentBossInputLocked(): boolean {
    ...
    if (this.gameMode === "strategySlice") {
      return this.strategySliceController?.inputLocked ?? false;
    }
    if (this.gameMode === "boss") {
  ...
  handlePointerMove(pos: Vec2) {
    ...
    if (!this.currentSlash?.active) {
      ...
      const activationDist = (this.gameMode === "bossReactive" || this.gameMode === "strategySlice") ? REACTIVE_BOSS_CONFIG.reactiveSlash.activationDistance : BALANCE.slash.activationDistance;
  ...
  private startSlash(pos: Vec2, pending?: NonNullable<typeof this.pendingSlash>) {
    ...
    const isReactiveMode = this.gameMode === "bossReactive" || this.gameMode === "strategySlice";
    ...
    this.currentSlash = {
      ...
      reactiveBladeEffect: isReactiveMode && this.reactiveController
        ...
        : isReactiveMode && this.strategySliceController
        ? { visualLength: 30 + (lockedMomentum?.ratio ?? 0) * 40, width: 3 + (lockedMomentum?.ratio ?? 0) * 5, brightness: 1, color: "#5bc0ff", glowColor: "#5bc0ff" }
        : undefined,
    };
  ...
  private checkSegmentHits(a: Vec2, b: Vec2, trail: SlashTrail) {
    ...
    if (this.gameMode === "strategySlice" && this.strategySliceController) {
      this.checkStrategySliceHits(a, b, trail);
      return;
    }
    const stage = SWORD_STAGE_BY_ID[trail.tier];
  ...
  private drawReactiveDebugOverlay(ctx: CanvasRenderingContext2D, rc: BossReactiveController): void {
    ...
    const lines = [
      ...
      // V0723016: 三甲目标与节奏遥测
      `[V0723016] bossElapsed: ${snap.bossElapsed.toFixed(1)}s`,
      `[V0723016] armorElapsed: ${snap.objectiveElapsed.toFixed(1)}s`,
      `[V0723016] objective: ${snap.objectiveType ?? "-"} ${snap.objectiveCurrent}/${snap.objectiveTarget}`,
      `[V0723016] completedBy: ${snap.objectiveCompletedBy ?? "-"}`,
      `[V0723016] perfectReflect: ${snap.perfectReflectCount}`,
      `[V0723016] mixedRound: ${snap.mixedRoundCount}`,
      `[V0723016] inputLockedSec: ${snap.inputLockedSeconds.toFixed(1)}`,
      `[V0723016] armorBreakTs: [${snap.armorBreakTimestamps.map(t => t.toFixed(1)).join(", ")}]`,
      `[V0723016] bridgeTs: ${snap.bridgeTimestamp?.toFixed(1) ?? "-"}`,
      // V0723016复审P0-3: 时间轴遥测
      `[Timeline] bossStart: ${snap.bossStartTs?.toFixed(1) ?? "-"}s`,
      `[Timeline] leftBreak: ${snap.leftBreakTs?.toFixed(1) ?? "-"}s`,
      `[Timeline] rightBreak: ${snap.rightBreakTs?.toFixed(1) ?? "-"}s`,
      `[Timeline] chestBreak: ${snap.chestBreakTs?.toFixed(1) ?? "-"}s`,
      `[Timeline] breakthrough: ${this._breakthroughSuccessTs?.toFixed(1) ?? "-"}s`,
    ];
    ctx.save();
  ...
  /** V0723016-S1: 策略切片弹幕/护甲碰撞检测 */
  private checkStrategySliceHits(a: Vec2, b: Vec2, trail: SlashTrail): void {
    const ssc = this.strategySliceController;
    if (!ssc) return;
    ...
    const visualLength = rEff ? rEff.visualLength : 30;
    const width = rEff ? rEff.width : 3;
    const geometry = buildReactiveSlashGeometry(a, b, this.lastSlashAngle, visualLength, width, trail.id, trail.lockedMomentum ?? createBladeMomentumState(this.energy, BLADE_MOMENTUM_CONFIG.baseMax));
    const bladeReach = geometry.effectiveRadius;

    // 检查弹幕碰撞
    for (const p of ssc.getProjectiles()) {
      if (!p.active || p.resolved) continue;
      for (const cap of geometry.capsules) {
        if (capsuleHitsEllipse(cap.a, cap.b, cap.radius, { x: p.x, y: p.y }, p.radius, p.radius)) {
          const momentum = trail.lockedMomentum ?? createBladeMomentumState(this.energy, BLADE_MOMENTUM_CONFIG.baseMax);
    ...
    // 检查护甲碰撞
    const armor = ssc.getArmorWorldPos();
    for (const cap of geometry.capsules) {
      if (capsuleHitsEllipse(cap.a, cap.b, cap.radius, { x: armor.cx, y: armor.cy }, armor.rx, armor.ry)) {
        const momentum = trail.lockedMomentum ?? createBladeMomentumState(this.energy, BLADE_MOMENTUM_CONFIG.baseMax);
        const dmg = ssc.resolveArmorHit(momentum);
        if (dmg > 0) {
          this.particles.push(...sparkBurst({ x: armor.cx, y: armor.cy }, 4, "#f0e130", 20));
          if (ssc.armorDurability <= 0) {
            this.particles.push(...explosionBurst({ x: armor.cx, y: armor.cy }, 3, ["#ffd35a", "#ff6a33"]));
          }
        }
        break;
      }
...
private finalizeBossSlashCommon(trail: SlashTrail): void {
  ...
  private drawHud(ctx: CanvasRenderingContext2D) {
    ...
    if (this.gameMode === "boss" || this.gameMode === "bossReactive" || this.gameMode === "strategySlice" || this.bossController) return;
  ...
  private initializeThunderGeneralBoss(): void {
    ...
    // V0723016复审P0-3: 重置突破成功时间轴
    this._breakthroughSuccessTs = null;
    if (this.gameMode === "bossReactive") {
