...
export class Game {
  ...
  private _eliteFireSubPhase: "queued" | "firing" | "done" = "queued"; // 0807-11E-1H: 环发射时序
  ...
  private _getFireRingPressureProfile(round: number) {
    ...
    if (r === 2) return { humanDur: 0.65, times: [0.16, 0.45, 0.72], dur: 0.98, gather: 0.20, reform: 0.28, gc: { x: 125, y: 345 }, tperm: [300, 80, 190] };
    return { humanDur: 0.65, times: [0.10, 0.32, 0.54], dur: 0.90, gather: 0.14, reform: 0.20, gc: { x: this._elitePressureRound % 2 === 1 ? 255 : 125, y: 345 }, tperm: [190, 300, 80] };
  }
  ...
  // 0807-11E-2A: 确定性dash
  private _eliteDashStart: { x: number; y: number } | null = null;
  private _eliteDashTarget: { x: number; y: number } | null = null;
  private _eliteDashDuration = 0;
  private _eliteDashElapsed = 0;
  // 0807-11E-2A: 人形/墨焰
  private _eliteForm: "human" | "morphing_to_flame" | "inkflame" | "reforming" = "human";
  private _eliteFormTimer = 0;
  // 0807-11E-2A: 火环计数防卡死
...
/** V0803027: 实时连击计数器 */
  ...
  private _comboSparks: { x: number; y: number; life: number; vx: number; vy: number; size: number }[] = [];
  private _comboBaseLife = 0; // 底座入场动画
  ...
  private static _TEST_VARIANTS: Record<string, string[]> = {
    ...
    shooter: ['DEFAULT','SPREAD3','SPREAD5','FAST','BURST'],
  };
  ...
  /** P0: 最近一次 Reactive 收刀结算结果（供 Debug 遥测显示，从 Controller 直接读 primaryResult/Source） */
  ...
  private updateBossMode(scaledDt: number, frameDt: number): void {
    ...
    if (this.bossController?.phase === "fail") {
      this.finish(false);
      return;
    }
  ...
  private extendSlash(pos: Vec2) {
    ...
    // 三刀流实时扩面：同步生成左右点 + 碰撞
    if (this._tripleLeftTrail?.active && this._tripleRightTrail?.active) {
      const halfLen = Math.max(1, Math.sqrt((pos.x-last.x)**2+(pos.y-last.y)**2));
      const nx = -(pos.y-last.y)/halfLen, ny = (pos.x-last.x)/halfLen;
      const progress = clamp(trail.pathUsed / Math.max(1, trail.maxPathLength), 0, 1);
      const ease = progress<0.5?2*progress*progress:1-Math.pow(-2*progress+2,2)/2;
      const off = 24 + (72 - 24) * ease;
      const pts = [{x:pos.x+nx*(-off),y:pos.y+ny*(-off)},{x:pos.x+nx*off,y:pos.y+ny*off}];
      for (let si=0;si<2;si++) {
        const t = si===0 ? this._tripleLeftTrail! : this._tripleRightTrail!;
        t.points.push({x:pts[si].x,y:pts[si].y,t:this.elapsed,energyRatio:ratio} as SlashPoint);
  ...
  private checkSegmentHits(a: Vec2, b: Vec2, trail: SlashTrail) {
    ...
    if (this.gameMode === "boss" && this.bossController) {
      ...
      } else if (this.bossController.phase === "pursuit") {
    ...
    for (const fr of this._eliteFireRings) {
      ...
      if (segmentHitCircle(a, b, { x: fr.x, y: fr.y }, fr.r + bladeReach + 6)) {
        ...
        if (r && r.isAccepted && r.effectiveHpLoss > 0) {
          ...
          // 0808-11E-4: 环碎直接伤Boss HP
          const frElite = this.enemies.find(e => e.kind === "elite" && e.eliteKind === "fireRing" && e.alive);
          if (frElite) {
  ...
  private checkReactiveEnemyProjectileHits(a: Vec2, b: Vec2, trail: SlashTrail): void {
    ...
    for (const ev of events) {
      if (ev.kind === "projectile_cut") {
        this.particles.push(...sparkBurst(ev.hitPos, 4, "#9b59b6"));
      } else if (ev.kind === "projectile_reflect") {
        this.particles.push(...sparkBurst(ev.hitPos, 6, "#ffd700"));
      } else if (ev.kind === "projectile_dangerous") {
        this.particles.push(...sparkBurst(ev.hitPos, 8, "#c0392b"));
      } else if (ev.kind === "armor") {
        this.particles.push(...sparkBurst(ev.hitPos, 4, "#f0e130"));
  ...
  private damageEnemy(enemy: Enemy, damage: number, trail: SlashTrail, chainKill: boolean, source: string) {
    ...
    // 0808-11E-4: 分裂循环中Boss始终可攻击
    if (enemy.eliteKind === "fireRing" && this._eliteBattleActive) {
      const locked = (this._eliteForm === "inkflame" || this._eliteForm === "morphing_to_flame");
      if (locked) {
        // 墨焰态: 刀穿火散开(无格挡语义)
    ...
    const hpBefore = enemy.hp;
    enemy.hp -= damage;
    // 0809-11E-5A-1: 实际HP损失+elite源
    if (enemy.eliteKind === "fireRing" && this._eliteBattleActive) {
      const actualLoss = hpBefore - Math.max(0, enemy.hp);
      if ((source === "paper" || source === "elite") && this._eliteBudgetHumanActive) {
        this._eliteBudgetHumanMain += actualLoss;
      } else if (this._eliteBudgetHumanActive) {
        this._eliteBudgetHumanOther += actualLoss;
      } else {
        this._eliteBudgetOther += actualLoss;
      }
    }
    enemy.flash = 0.25;
    if (enemy.hp <= 0) {
  ...
  private killEnemy(enemy: Enemy, trail: SlashTrail, chainKill: boolean, source: string) {
    ...
    if (enemy.eliteKind === "fireRing" && this._eliteBattleActive) {
      ...
      const unattributed = (this._eliteBudgetHpStart) - total;
      console.log(`[ELITE ROUND BUDGET] DEATH R=${this._elitePressureRound} hpStart=${this._eliteBudgetHpStart} humanMain=${this._eliteBudgetHumanMain} humanOther=${this._eliteBudgetHumanOther} ring=${this._eliteBudgetRing} other=${this._eliteBudgetOther} unattributed=${unattributed} hpEnd=0`);
    }
    enemy.alive = false;
    trail.kills += 1;
...
private finalizeBossSlashCommon(trail: SlashTrail): void {
  ...
  if (this.reactiveController) {
    ...
    // P0: 从 result.events 做视觉反馈（粒子/文字），不再扫描 projectiles
    const cfg = REACTIVE_BOSS_CONFIG.bladeEnergy;
    for (const ev of result.events) {
      if (ev.kind === "projectile_cut") {
        this.particles.push(...sparkBurst(ev.hitPos, 6, "#9b59b6"));
        this.addText(ev.hitPos.x, ev.hitPos.y - 12, `+${cfg.normalBulletReward}`, "#5bc0ff", 12);
      } else if (ev.kind === "projectile_reflect") {
        ...
        const fx = ap?.x ?? ev.hitPos.x;
        const fy = ap?.y ?? ev.hitPos.y;
        this.flash = Math.max(this.flash, 0.35);
  ...
  private applyArmorResolveResult(result: import("./systems/BossController").ArmorResolveResult, segA: Vec2, segB: Vec2): void {
    if (result.kind === "armor_hit") {
      ...
      // 第三击用armorComplete替代armorHit，避免叠加
      if (result.completed) {
  ...
  private applyExecutionResolveResult(result: ExecutionResolveResult, _segA: Vec2, _segB: Vec2): void {
    if (result.kind === "execution_hit") {
      ...
      this.screenShake = Math.max(this.screenShake, 0.5);
      this.particles.push(...sparkBurst({ x: result.hitPos.x, y: result.hitPos.y }, 15, "#f0e130"));
      this.particles.push(...sparkBurst({ x: result.hitPos.x, y: result.hitPos.y }, 10, "#ffffff"));
      AudioService.armorComplete();
      this.addCombatFloat({
        x: result.hitPos.x, y: result.hitPos.y - 20,
      ...
      this.particles.push(...sparkBurst({
        x: corePos.cx, y: corePos.cy
      }, 8, "#c0392b"));
      this.addCombatFloat({
        x: DESIGN_WIDTH / 2, y: 250,
        text: "斩空...", color: "#ff6a33", size: 18, duration: 0.8,
        category: "mechanic", priority: "A", mergeKey: `execution-miss-${result.slashId}`
      });
    }
  }
  ...
  private _detonatePowder(enemy: Enemy) {
    ...
    const expX = enemy.x, expY = enemy.y;
    // 0807-11D-6F-6: 专用爆心 (由_drawExplosionShockwaves绘制)
    this._explosionShockwaves.push({ x: expX, y: expY, startTime: this.elapsed, maxRadius: 48, color: "#ffffff", isCore: true } as any);
    this._explosionShockwaves.push({ x: expX, y: expY, startTime: this.elapsed, maxRadius: radius * 0.85, color: "#ff4400" });
    this._explosionShockwaves.push({ x: expX, y: expY, startTime: this.elapsed + 0.02, maxRadius: radius * 1.05, color: "#ff8844" });
    for (let j = 0; j < 30; j++) {
    ...
    for (const target of this.enemies) {
      ...
      // 0807-11D-6F-6: 不再直接arming, 伤害由damageEnemy处理→HP归零→统一入口
    }
    // 玩家伤害: 使用固定战斗位而非pointerPos
    const px = DESIGN_WIDTH / 2;
    const py = BALANCE.battlefield.warriorY;
  ...
  private drawEliteShieldLayer(ctx: CanvasRenderingContext2D, enemy: Enemy) {
    ...
    if (enemy.eliteKind === "fireRing") { return; } // 0808-11E-4: 走正常human绘制
  ...
  private _drawFireRingEliteVisual(ctx: CanvasRenderingContext2D, elite: Enemy) {
    ...
    if (f === "inkflame" || f === "morphing_to_flame") {
      ...
      const flameH = r * 3.2;
      // 0808-11E-3B: 破环削弱亮度
      const broken = 0; // 0808-11E-4: inkflame已不再用于fireRing
  ...
  private _addFragment(x: number, y: number, result: "broken" | "touched") {
    if (this._eliteFragments.length > 20) return;
    for (let i = 0; i < 4; i++) {
      const angle = Math.random() * Math.PI * 2;
      const dist = Math.random() * 15;
      this._eliteFragments.push({
        x: x + Math.cos(angle) * dist, y: y + Math.sin(angle) * dist,
        vx: Math.cos(angle) * 40, vy: Math.sin(angle) * 40 - 20,
        size: 3 + Math.random() * 4, alpha: 0.9,
        bright: result === "broken", result, srcX: x, srcY: y, gathered: false,
      });
    }
  }

  private _drawEliteFragments(ctx: CanvasRenderingContext2D) {
    for (const frag of this._eliteFragments) {
      if (frag.gathered) continue;
      frag.y += frag.vy * 0.016;
      frag.alpha *= 0.995;
      ctx.fillStyle = frag.bright
        ? `rgba(255,220,120,${frag.alpha})`
        : `rgba(180,100,40,${frag.alpha})`;
      ctx.beginPath(); ctx.arc(frag.x, frag.y, frag.size, 0, Math.PI * 2); ctx.fill();
    }
  ...
  private _spawnFireRing(elite: any, targetX: number, waveId: number) {
    this._eliteRingIssued++;
    this._eliteFireRings.push({
      x: elite.x + (targetX - 190) * 0.15, y: elite.y + 15, r: 24,
      speed: 220 + Math.random() * 40, alive: true, hasHit: false, hp: 1,
      targetX, targetY: BALANCE.battlefield.bottomDefenseY, _waveId: waveId,
    });
  }

  private _startDash(elite: any, target: { x: number; y: number }, duration: number) {
    this._eliteDashStart = { x: elite.x, y: elite.y };
    this._eliteDashTarget = target;
    this._eliteDashDuration = duration;
    this._eliteDashElapsed = 0;
  }
  ...
  private _drawFrostSlash(ctx: CanvasRenderingContext2D, points: any[], width: number, lowFade: number, effBrightness: number) {
    ...
    ctx.save();
    ctx.globalAlpha=lowFade*0.25; ctx.strokeStyle='rgba(150,210,240,0.5)'; ctx.lineWidth=4;
    ctx.setLineDash([20, 28]); ctx.beginPath();
    ctx.moveTo(points[0].x,points[0].y);
    for(let i=1;i<n-1;i++)ctx.lineTo(points[i].x,points[i].y);ctx.stroke();
    ctx.setLineDash([]);
    ctx.globalAlpha=lowFade*0.3; ctx.fillStyle='#cce';
    const sb=(points[0].x*137+points[0].y*251)%10000;
    for(let i=0;i<5;i++){
      const idx=Math.floor((i+0.5)*(n-1)/5); if(idx>=n)continue;
      const p=points[idx], s=sb+i*173, a=Math.sin(s)*Math.PI;
      ctx.beginPath();
      ctx.moveTo(p.x+Math.cos(a)*4,p.y+Math.sin(a)*4);
      ctx.lineTo(p.x+Math.cos(a+0.4)*10,p.y+Math.sin(a+0.4)*10);
  ...
  private _drawSlashShape(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, progress: number): void {
    ...
    const notch = h * 0.6;
    ctx.moveTo(x, y);
    ctx.lineTo(x + pw - 4, y - notch * 0.3);
    ctx.lineTo(x + pw, y + h * 0.3);
    ctx.lineTo(x + pw - 2, y + h);
  ...
  private _drawChestOpeningFlow(ctx: CanvasRenderingContext2D) {
    ...
    if (this._chestOpeningPhase === "exiting" && this._pendingEdictId) {
      ...
      ctx.globalAlpha = a;
      ctx.shadowColor = meta.glow; ctx.shadowBlur = 20;
      ctx.font = `${Math.round(48 * scale)}px "Microsoft YaHei", sans-serif`;
      ctx.textAlign = "center";
  ...
  private _drawScorchGround(ctx: CanvasRenderingContext2D) {
    for (const vr of this._scorchVisualRoads) {
      ...
      const pts=vr.points,n=pts.length,gf=Math.min(1,vr.life/(vr.maxLife*0.6));if(gf<0.02)continue;
      ctx.save();
      // 连续赤橙火带 48-60px
      ctx.globalAlpha=gf*0.14; ctx.strokeStyle="#771100"; ctx.lineWidth=54;
      ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);for(let i=1;i<n;i++)ctx.lineTo(pts[i].x,pts[i].y);ctx.stroke();
      ctx.globalAlpha=gf*0.09; ctx.strokeStyle="#aa2200"; ctx.lineWidth=36;
      ctx.beginPath();ctx.moveTo(pts[0].x,pts[0].y);for(let i=1;i<n;i++)ctx.lineTo(pts[i].x,pts[i].y);ctx.stroke();
      ctx.restore();
    }
  }
  ...
  private _drawFrostEffects(ctx: CanvasRenderingContext2D) {
    for (const enemy of this.enemies) {
      ...
      if (isFrozen) {
        // 冰壳包住敌人
        ctx.globalAlpha=0.4; ctx.fillStyle="rgba(130,200,240,0.45)";
        ctx.strokeStyle="rgba(220,240,255,0.95)"; ctx.lineWidth=3*scale;
        const sides=6; ctx.beginPath();
        for(let i=0;i<sides;i++){const ang=i*Math.PI*2/sides+seed*0.01;
        const dist=r+5*scale+Math.sin(i*2.3+seed)*2;
        i===0?ctx.moveTo(enemy.x+Math.cos(ang)*dist,enemy.y+Math.sin(ang)*dist)
        :ctx.lineTo(enemy.x+Math.cos(ang)*dist,enemy.y+Math.sin(ang)*dist);}
        ctx.closePath();ctx.fill();ctx.stroke();
        // 内部冰裂纹
        ctx.globalAlpha=0.35;ctx.strokeStyle="rgba(200,235,255,0.6)";ctx.lineWidth=1.5;
        for(let i=0;i<3;i++){const a=seed*0.03+i*2.1;
        ctx.beginPath();ctx.moveTo(enemy.x+Math.cos(a)*r*0.3,enemy.y+Math.sin(a)*r*0.3);
        ctx.lineTo(enemy.x+Math.cos(a+0.4)*r*0.7,enemy.y+Math.sin(a+0.4)*r*0.7);ctx.stroke();}
        // 冰屑
        ctx.globalAlpha=0.4;ctx.fillStyle="#eef";
        for(let i=0;i<4;i++){const a=seed*0.1+i*Math.PI/2;const d=r+7*scale;
        ...
        // 飞散冰块 8-14px
        ctx.globalAlpha=0.5+sp*0.2;ctx.fillStyle="#ddeeff";
        for(let i=0;i<8;i++){const a=seed*0.07+i*Math.PI/4;
        const d=r+6+sp*24,sz=4+(seed+i*7)%10;
        ctx.beginPath();
        ctx.moveTo(enemy.x+Math.cos(a)*d,enemy.y+Math.sin(a)*d);
        ctx.lineTo(enemy.x+Math.cos(a+0.25)*(d+sz),enemy.y+Math.sin(a+0.25)*(d+sz));
  ...
  private drawEnemies(ctx: CanvasRenderingContext2D) {
    ...
    for (const enemy of sorted) {
      ...
      if (enemy.kind === "powder") {
        // 0807-11D-6F-6: 4次确定性脉冲 + 整体缩放
        const t = enemy._fuseTimer || 0;
        const inFuse = enemy._fuseState === 'arming';
        if (inFuse) {
          // 4次脉冲: 0.0~0.30 / 0.30~0.55 / 0.55~0.76 / 0.76~0.96
          let fuseScale = 1.0;
          const compress = t > 0.96 ? Math.min(1, (t - 0.96) / 0.12) : 0;
          if (t < 0.30) { const p = t / 0.30; const s = Math.sin(p * Math.PI); fuseScale = 1.0 + 0.24 * s; }
          else if (t < 0.55) { const p = (t - 0.30) / 0.25; const s = Math.sin(p * Math.PI); fuseScale = 1.06 + 0.28 * s; }
          else if (t < 0.76) { const p = (t - 0.55) / 0.21; const s = Math.sin(p * Math.PI); fuseScale = 1.10 + 0.36 * s; }
          else if (t < 0.96) { const p = (t - 0.76) / 0.20; fuseScale = 1.14 + 0.46 * p; } // 只涨不落
          // 压缩: 1.60→0.70
          if (compress > 0) fuseScale = 1.60 * (1 - compress) + 0.70 * compress;
          // 颜色渐变
          const urgency = Math.min(t / 0.96, 1);
          const cr = Math.round(240 - urgency * 30), cg = Math.round(160 - urgency * 40), cb = Math.round(50 + urgency * 100);
          // 保存→整体缩放→绘制
          ctx.save();
          ctx.scale(fuseScale, fuseScale);
          // 外围橙色危险光晕
          const glowRadius = enemy.radius * (1.2 + urgency * 0.8);
          const glowGrad = ctx.createRadialGradient(0, 0, enemy.radius * 0.6, 0, 0, glowRadius);
          glowGrad.addColorStop(0, 'rgba(255,160,40,0)');
          glowGrad.addColorStop(0.6, `rgba(255,120,30,${0.10+urgency*0.25})`);
          ...
          ctx.shadowColor = `rgba(255,${180-Math.round(urgency*60)},30,${0.2+urgency*0.6})`;
          ctx.shadowBlur = 6 + urgency * 18;
          ctx.shadowOffsetY = 2;
          ctx.fillText("爆", 0, 0);
          ctx.shadowBlur = 0; ctx.shadowOffsetY = 0;
          // 火星
          const sparkCount = 4 + Math.floor(urgency * 5);
          for (let i = 0; i < sparkCount; i++) {
            const a = urgency * 6 + i * 1.3;
            const sr = glowRadius * 0.8;
            ctx.fillStyle = `rgba(255,${150-Math.round(urgency*50)},30,${0.3+urgency*0.4})`;
            ctx.beginPath(); ctx.arc(Math.cos(a)*sr, Math.sin(a)*sr, 3, 0, Math.PI*2); ctx.fill();
          }
          // 压缩内吸粒子
          if (compress > 0) {
            for (let j = 0; j < 5; j++) {
              const da = j * Math.PI * 0.4, dr = glowRadius * (1 - compress * 0.8);
              ctx.fillStyle = `rgba(255,200,60,${compress*0.7})`;
              ctx.beginPath(); ctx.arc(Math.cos(da)*dr, Math.sin(da)*dr, 3, 0, Math.PI*2); ctx.fill();
            }
          }
          ctx.restore();
        } else {
  ...
  private _drawExplosionShockwaves(ctx: CanvasRenderingContext2D) {
    for (const s of this._explosionShockwaves) {
      ...
      // 0807-11D-6F-6: 爆炸核心 0~0.08s
      if ((s as any).isCore && elapsed <= 0.08) {
        const coreP = elapsed / 0.08;
        const coreR = 12 + coreP * 36; // 12→48px
        const alpha = 1 - coreP;
        // 白色实亮核心
        ctx.save();
        ctx.fillStyle = `rgba(255,255,255,${alpha})`;
        ctx.beginPath(); ctx.arc(s.x, s.y, coreR * 0.5, 0, Math.PI*2); ctx.fill();
        // 黄白radialGradient外层
        const grad = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, coreR);
        grad.addColorStop(0, `rgba(255,255,220,${alpha})`);
        grad.addColorStop(0.5, `rgba(255,200,60,${alpha*0.7})`);
  ...
  private _drawDebugCompact(ctx: CanvasRenderingContext2D): void {
    ...
    const shadowMove = this.enemies.filter(e => e.alive && e._directorEntryState === 'shadow_move').length;
    ...
    const rowData = [
      ...
      ...(this._showHpOverlay ? [
        ...
        ...(hasDirector ? [
          ...
          { l: 'shadow', v: `M${shadowMove} G${matCount} A${activeCount}`, c: '#7ec8e3' },
  ...
  private _updateEliteBattle(dt: number) {
    ...
    // 0807-11E-2A: 确定性dash插值(easeInOutCubic)
    if (this._eliteDashTarget) {
      this._eliteDashElapsed += dt;
      const t = Math.min(1, this._eliteDashElapsed / this._eliteDashDuration);
      const v = t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
      elite.x = this._eliteDashStart!.x + (this._eliteDashTarget.x - this._eliteDashStart!.x) * v;
      elite.y = this._eliteDashStart!.y + (this._eliteDashTarget.y - this._eliteDashStart!.y) * v;
      if (t >= 1) { this._eliteDashTarget = null; this._eliteDashStart = null; }
    }
    // 形态过渡
    ...
    this._eliteSeqTimer += dt;
    const goSeq = (next: typeof this._eliteSeq) => { this._eliteSeq = next; this._eliteSeqTimer = 0; };

    // 碎片gather
    if (this._eliteGatherPhase === "attract") {
      this._eliteGatherTimer += dt;
      for (const frag of this._eliteFragments) {
        if (frag.gathered) continue;
        const dx = this._eliteGatherCenter.x - frag.x;
        const dy = this._eliteGatherCenter.y - frag.y;
        const dist = Math.hypot(dx, dy);
        if (dist < 8) { frag.gathered = true; continue; }
    ...
    switch (this._eliteSeq) {
      ...
      case "wait_resolve": {
        ...
        if (forceEnd && resolved < 3) {
          ...
          for (const fr of this._eliteFireRings) {
            ...
            this._addFragment(fr.x, fr.y, "touched");
          }
        }
      ...
      case "gather": {
        this._eliteCyclePhase = "fire"; this._eliteForm = "human";
        break;
      }
      case "reform": {
        ...
        if (this._eliteSeqTimer >= this._eliteActivePf.reform) {
          ...
          const total = this._eliteBudgetHumanMain + this._eliteBudgetHumanOther + this._eliteBudgetRing + this._eliteBudgetOther;
          const unattributed = (this._eliteBudgetHpStart - hpEnd) - total;
          console.log(`[ELITE ROUND BUDGET] R=${this._elitePressureRound} hpStart=${this._eliteBudgetHpStart} humanMain=${this._eliteBudgetHumanMain} humanOther=${this._eliteBudgetHumanOther} ring=${this._eliteBudgetRing} other=${this._eliteBudgetOther} unattributed=${unattributed} hpEnd=${hpEnd}`);
