...
type Screen = "menu" | "battle" | "result" | "upgrades" | "forge" | "challenge" | "team" | "ranking" | "idle" | "bag" | "debug" | "breakthroughResult" | "bossFailScreen" | "sliceResult";
...
export default function App() {
  ...
  /** V0723016-S1.2: 策略切片结果 */
  const [sliceResult, setSliceResult] = useState<BattleResult["strategySliceSummary"] | null>(null);
  ...
  const handleFinish = useCallback(
    (result: BattleResult) => {
      ...
      refreshHome();
      window.setTimeout(() => {
        // V0723016-S1.2: 策略切片结果（不走突破/普通结果）
        if (result.strategySlice) {
          setSliceResult(result.strategySliceSummary ?? null);
          setScreen("sliceResult");
          return;
        }
  ...
  return (
    <main className="app-shell">
      ...
      {/* V0723016-S1.2: 策略切片结果页 */}
      {screen === "sliceResult" && sliceResult && (
        <section style={{ padding: "40px 24px", textAlign: "center", minHeight: "100vh", background: "linear-gradient(180deg, #0a0614 0%, #140f1c 100%)" }}>
          <h2 style={{ fontSize: 26, color: "#ffd35a", marginBottom: 12, textShadow: "0 0 12px rgba(255,211,90,0.3)" }}>策略切片完成</h2>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)", marginBottom: 20 }}>总耗时 {sliceResult.totalTime?.toFixed(1)}s</p>
          {/* S1.5: 每轮路径 */}
          {sliceResult.cycleOutcomes && sliceResult.cycleOutcomes.length > 0 && (
            <div style={{ maxWidth: 260, margin: "0 auto 20px", textAlign: "left" }}>
              {sliceResult.cycleOutcomes.map((outcome, i) => (
                <div key={i} style={{ padding: "4px 0", fontSize: 13, color: "rgba(255,255,255,0.6)" }}>
                  第{i + 1}轮：
                  <span style={{ color: outcome === "safe_clear" ? "#5bc0ff" : outcome === "charged_reflect" ? "#ffd35a" : "#ff3535", fontWeight: 600 }}>
                    {outcome === "safe_clear" ? "安全清场" : outcome === "charged_reflect" ? "充能反射" : "过载"}
                  </span>
                </div>
              ))}
            </div>
          )}
          <div style={{ maxWidth: 260, margin: "0 auto 40px", textAlign: "left" }}>
            {[
              ["安全清场", sliceResult.cleanClears],
              ["充能反射", sliceResult.chargedReflects],
              ["过载", sliceResult.overloads],
              ["窗口攻击", sliceResult.windowAttacks],
              ["放弃窗口", sliceResult.windowSkips],
              ["危险误砍", sliceResult.dangerWrongCuts],
            ].map(([label, val]) => (
              <div key={label as string} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.7)", fontSize: 15 }}>
                <span>{label}</span>
                <span style={{ color: "#ffd35a", fontWeight: 700 }}>{val}</span>
              </div>
            ))}
          </div>
          {/* S1.6: 策略对比指标 */}
          <div style={{ maxWidth: 260, margin: "0 auto 40px", textAlign: "left", color: "rgba(255,255,255,0.6)", fontSize: 13 }}>
            {sliceResult.totalArmorDamage !== undefined && (
              <div style={{ padding: "3px 0" }}>护甲总伤害: <span style={{ color: "#ffd35a", fontWeight: 600 }}>{sliceResult.totalArmorDamage}</span></div>
            )}
            {sliceResult.remainingArmor !== undefined && (
              <div style={{ padding: "3px 0" }}>护甲剩余: <span style={{ color: "#ff6a33" }}>{sliceResult.remainingArmor}</span></div>
            )}
          ...
          <br />
          <button className="battle-btn" style={{ width: 200, background: '#2c1038', border: '1px solid #4a2060' }} onClick={() => { setSliceResult(null); setCurrentMode("normal"); setScreen("menu"); }}>
            返回首页
          </button>
        </section>
      )}
